...
function render(){playlist.innerHTML='';tracks.forEach((t,n)=>{const e=document.createElement('div');e.className='song';e.innerHTML=`<span>${n+1}. ${String(t.title||t.name||'Track').replace(/[<>]/g,'')}</span><small>${t.type==='youtube'?'YouTube':'à¹€à¸žà¸¥à¸‡à¸ˆà¸²à¸à¹€à¸„à¸£à¸·à¹ˆà¸­à¸‡'}</small>`;e.onclick=()=>setTrack(n,true);playlist.appendChild(e)})}
